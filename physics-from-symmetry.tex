...
\begin{equation}
X=\left.\deriv{h(\theta)}{\theta}\right|_{\theta=0}.
\end{equation}
...
\begin{thm}
Consider a matrix Lie group $G$ and corresponding Lie algebra $g$. If $X, Y \in \mathfrak{g}$ and $g, h \in G$, then
\begin{equation}
g \circ h = e^X \circ e^Y = \underbrace{e^{X + Y + \frac{1}{2}[X,Y]+\frac{1}{12}[X,[X,Y]]-\frac{1}{12}[Y,[X,Y]]+...}}_{\in G}
\end{equation}
where the right hand side is the Baker-Campbell-Hausdorff formula, and $[X,Y]$ is the \textbf{commutator} of $X$ and $Y$
\begin{equation}
[X,Y] = XY - YX
\end{equation}
and the commutator corresponds to the Lie bracket.
\end{thm}
...
\end{equation}
A set of three matrices $J_1, J_2, J_3$ fulfilling both of these conditions can be written conveniently as
\begin{equation}
