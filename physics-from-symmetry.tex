...
Moving from left to right, the first index denotes a row, the second index denotes a column, and so on. Hence
\begin{equation}
(\textbf{A})_{ij} \defeq \tensor{A}{^i_j}
\end{equation}
where the right hand side is Einstein notation, and the left hand side is usual matrix element notation. ``Upper indicies go up to down; lower indicies go left to right''. We commonly use the conventions:
\begin{itemize}
\item Greek letters are used for spacetime components, where indicies take values 0, 1, 2, or 3.
\item Latin letters are used for spatial components only, where indicies take values 1, 2, or 3.
\end{itemize}
...
\paragraph{Raising/lowering with metric tensor} In order to be able to freely raise/lower indicies (without repeatedly writing the metric tensor), we impose an ordering upon indicies of tensor fields -- which we represent typographically with spacing between tensor indicies. A metric $g_{ij}$ (or $g^{ij}$) has the effect of lowering (or raising) a repeated index. For example,
\begin{equation}
...
\eta_{\mu \nu} &= \tensor{(\Lambda^T \eta \Lambda)}{^\mu_\nu} \nonumber \\
...
\begin{equation}
J_i \equiv \sigma_i/2 \label{eq:generator-su2-lie-algebra}
\end{equation}
